...
> 9. **Lint gates**：dead import / unused var / param **由 tsc 把關**（後端 `tsconfig.json` + `tsconfig.test.json` + frontend `tsconfig.json` 三組皆開 `noUnusedLocals` + `noUnusedParameters`，CI 既有 `tsc --noEmit` step 自動 gate）。**前端 ESLint** 只裝 `react-hooks` plugin、規則只留 `exhaustive-deps`（warn）抓 stale closure；v7 加的 `set-state-in-effect` / `purity` / `refs` / `no-undef` 對 React 18 codebase 過嚴，已 off。**後端不裝 ESLint**（純邏輯走 tsc strict + tests 噪音 > 價值）。新增 plugin 前先問「tsc 能不能抓」。
>
