...
-- Baseline migration. Captures the prod schema as of 2026-05-11.
--
-- Every statement uses `IF NOT EXISTS` / `INSERT OR IGNORE`, so first apply
-- on prod (which already has every table from prior `wrangler d1 execute
-- --file src/db/schema.sql` runs) is a no-op. `wrangler d1 migrations apply`
-- then records this file in the `d1_migrations` tracking table so future
-- migrations (0002_*, 0003_*, ...) only run their own statements.
--
-- Future schema changes: create `migrations/000N_short_name.sql` with the
-- DELTA only — no `CREATE TABLE IF NOT EXISTS` for tables that already
-- exist in 0001. ALTER TABLE doesn't accept `IF NOT EXISTS` in SQLite, so
-- a per-column migration is one statement; deploy CI applies it once and
-- the d1_migrations row keeps it from re-running.
